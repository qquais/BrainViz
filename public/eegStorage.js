...
class EEGStorage {
  ...
  async storeEDFFile(arrayBuffer, filename = 'uploaded.edf') {
    try {
      console.log('💾 Storing EDF file in IndexedDB:', filename, arrayBuffer.byteLength, 'bytes');
  ...
  async getEDFFile() {
    try {
      ...
      const result = await new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => {
          if (request.result) {
            console.log('✅ EDF file retrieved:', request.result.filename, request.result.size, 'bytes');
          } else {
            console.log('ℹ️ No EDF file found in IndexedDB');
          }
          resolve(request.result);
        };
        request.onerror = () => {
  ...
  async storeTextFile(textData, filename = 'uploaded.txt') {
    try {
      console.log('💾 Storing text file:', filename, textData.length, 'characters');
      
      if (textData.length < 5000000) { // 5MB text limit
        return new Promise((resolve, reject) => {
  ...
  async clearAllData() {
    try {
      console.log('🗑️ Clearing all EEG data...');
  ...
  async getStorageInfo() {
    try {
      ...
      return {
        ...
        totalSize: totalSize,
        files: files.map(f => ({ id: f.id, filename: f.filename, size: f.size, type: f.type }))
      };
    } catch (error) {
